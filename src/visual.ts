...
export class Visual implements IVisual {
    ...
    private onBusquedaInput(): void {
        ...
        this.debounceTimer = window.setTimeout(() => {
            this.aplicarFiltro(
                this.inputBusqueda.value.trim().toLowerCase()
            );
        }, 120);
    }
    private aplicarFiltro(termino: string): void {
        if (!termino) {
            ...
            this.filasFiltradas = this.filas.filter(
                f => f.textoBusqueda.indexOf(termino) !== -1
            );
        }
        this.contador.textContent =
            this.filasFiltradas.length +
            " de " +
            this.filas.length +
            " clientes";
    ...
    public update(options: VisualUpdateOptions): void {
        ...
        this.columnas = dataView.table.columns.map(
            c => c.displayName
        );
    ...
    private actualizarAltoSpacer(): void {
        this.listaSpacer.style.height =
            (this.filasFiltradas.length * ALTO_FILA) + "px";
    }
    private renderizarVentana(): void {
        const alturaViewport =
            this.listaViewport.clientHeight || 1;
        const scrollTop =
            this.listaViewport.scrollTop;
        const primeraVisible =
            Math.floor(scrollTop / ALTO_FILA);
        const cantidadVisible =
            Math.ceil(alturaViewport / ALTO_FILA);
        const inicio =
            Math.max(0, primeraVisible - BUFFER_FILAS);
        const fin =
            Math.min(
                this.filasFiltradas.length,
                primeraVisible +
        ...
        this.listaVentana.style.transform =
            "translateY(" +
            (inicio * ALTO_FILA) +
            "px)";
